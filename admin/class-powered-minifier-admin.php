...
class Powered_Minifier_Admin
{
	...
	*/
	...
	{
		$html = '<div class="wpdx-header"><span class="header-icon"><i class="fas fa-sliders-h"></i></span> <span class="header-text">'.__('Powered Minifier', 'powered-minifier').'</span></div>';
		return $html;
	}
	...
	public function return_minified_output()
	{
		...
		if((isset($opts['source'])) && ($opts['source'] === 'on'))
		{
			if(isset($_SERVER['REQUEST_URI']))
			{
				...
				$base = basename($path);
				if($base && preg_match('/\.(html?|php|xml|json)$/i', $base))
				{
					return;
				}
			}
